...
function setup() {
  ...
  backgr=createSprite(0,0,displayWidth,displayHeight-100);
  ...
  zombie = createSprite(displayWidth-120, displayHeight-400,10,10); 
  zombie.addAnimation("zombieAttacking", zombieImg); 
  zombie.scale = 0.3; 


  gameover = createSprite(600,500,100,200) ;
...
function draw() {
 ...
 if(gameState === PLAY){
  ...
  reset()
   ...
   // if(zombie.isTouching(peopleGroup)){
    //  people.addAnimation("zombieAttacking", zombieImg);
    //}
...
function spawnVaccines() {
  if(frameCount % 100 === 0) {
    ...
    var vaccine = createSprite(r1,100,10,40);
    //vaccine.velocityX = -6;
    switch(r2){
...
function spawnPeople() {
  if(frameCount % 200 === 0) {
    ...
    people = createSprite(displayWidth-350,340,10,10); 
    switch(r4){
...
function spawnZombies() {
  if(frameCount % 200 === 0) {
    r5 = random(1,displayWidth/4); 
    zombie = createSprite(displayWidth-120, 400,10,10); 
    zombie.addAnimation("zombieAttacking", zombieImg); 
    zombie.velocityX = -2;
    zombie.scale = 0.3; 
    zombie.lifetime = 220;
